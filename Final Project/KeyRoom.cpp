...
Date: 6/6/2017
Description: the KeyRoom class - inherits from Space - Contains functions to move within the boss room and check for obstacles and special spaces.
******************************************************/

#include "KeyRoom.hpp"

// constructs the key room
KeyRoom::KeyRoom()
{
	playerX = 2;
	playerY = 3;
	...
	board[1][2] = "K";	// monster room
	board[4][2] = "D"; // door

}

// prints the key room
void KeyRoom::printBoard()
{
	for (int i = 0; i < 5; i++)
	{
		for (int j = 0; j < 5; j++)
...
bool KeyRoom::isWall(int x, int y)
{
...
// returns true if player steps on the key space
bool KeyRoom::specialSpace(int x, int y)
{
	if (y == 1 && x == 2)
	{
		cout << "The door to the Boss room has been unlocked." << endl;
		return true;
	}
	return false;
}
